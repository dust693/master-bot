...
from indicators.signal_combiner import add_combined_signal
...
def calculate_all_indicators(df: pd.DataFrame, strategy_cfg: dict) -> pd.DataFrame:
    ...
    # ==========================================
    # 5. ΣΥΝΔΥΑΣΜΟΣ ΣΗΜΑΤΩΝ -> ΤΕΛΙΚΗ ΣΤΗΛΗ 'signal'
    # ==========================================
    # Μέχρι εδώ έχουν υπολογιστεί τα επιμέρους signal columns (SMA_signal,
    # RSI_signal, MACD_pos_signal, ...) για όσους δείκτες είναι ενεργοί.
    # Το add_combined_signal() τα συνδυάζει σε ΜΙΑ τελική στήλη 'signal'
    # (+1 BUY, -1 SELL, 0 HOLD) σύμφωνα με το strategy_cfg["COMBINATION_LOGIC"]
    # (AND/OR/MAJORITY). Αυτή η στήλη 'signal' χρησιμοποιείται:
    #   - Από το backtest engine (candle-by-candle προσομοίωση)
    #   - Από τον StrategyManager (live/paper trading, τελευταίο κερί)
    df = add_combined_signal(df, strategy_cfg)

    # =============================================================================
    # ΕΠΙΣΤΡΟΦΗ DataFrame
    # =============================================================================
    # Επιστρέφει το df εμπλουτισμένο με όλες τις στήλες δεικτών, τα επιμέρους
    # *_signal ανά δείκτη, ΚΑΙ τη συνδυασμένη στήλη 'signal'.
    return df
